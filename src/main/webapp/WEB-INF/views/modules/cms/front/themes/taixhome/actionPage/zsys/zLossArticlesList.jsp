      ...
      <div class="zpxx_box clear">
        <ul>
        	...
        	<c:forEach items="${page.list}" var="zLossArticles">
	        	<a href="${ctx}/zsys/zLossArticles/formDetail?id=${zLossArticles.id}">
		            <li class="bbm">
		              <span class="zpxx_gyys xq">车牌号码：${zLossArticles.vehicleNum}</span>
		              <span class="zpxx_gyys gsmc">登记时间：<fmt:formatDate value="${zLossArticles.regDate}" pattern="yyyy-MM-dd HH:mm:ss"/></span>
		              <span class="zpxx_gyys dz"><fmt:formatDate value="${zLossArticles.updateDate}" pattern="yyyy.MM.dd"/></span>
		            </li>
	          	</a>
          	</c:forEach>
        ...
        <div class="fy_box">
          <ul>
            ...
            <script type="text/javascript">
				function page(n,s){
					location="${ctx}/zsys/zLossArticles/list?pageNo="+n+"&pageSize="+s;
				}
			</script>
          </ul>
        </div>
